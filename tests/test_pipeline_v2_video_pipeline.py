...
class VideoPipelineEndToEndTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_cache_version_2_12_0_invalidates_2_10_0_manifest(self):
        """VFR decoding and final acceptance changes invalidate old artifacts."""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            source = root / "source.mp4"
            source.write_bytes(b"source-video")
            ...
            # Create an existing manifest under version 2.10.0.
            with mock.patch(
                "backend.pipeline_v2.video_pipeline.PIPELINE_IMPLEMENTATION_VERSION",
                "2.10.0",
            ):
            ...
            self.assertEqual(manifest_v28.metadata["pipeline_implementation_version"], "2.12.1")
            self.assertNotEqual(
