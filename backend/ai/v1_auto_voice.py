...
def estimate_f0_pitch(y: np.ndarray, sr: int) -> Tuple[float, int, float, str]:
    """
    Uoc luong cao do co ban (F0) su dung Parselmouth (Praat) voi fallback sang librosa.yin.
    Tra ve: (median_f0, voiced_frame_count, confidence, gender)
    """
    ...
    try:
        ...
        # Dung to_pitch_cc voi voicing_threshold=0.45 de bo qua cac doan vo thanh
    ...
    if len(voiced_f0) == 0:
        try:
            ...
            if flatness > 0.4:  # White noise / noise
                return 0.0, 0, 0.0, "unknown"
    ...
    if len(voiced_f0) < MIN_VOICED_FRAMES:
        return 0.0, len(voiced_f0), 0.0, "unknown"
    ...
    return median_f0, voiced_count, confidence, gender
...
def detect_first_speaker_gender(
    ...
    max_search_time_sec: float = 25.0,
) -> Tuple[str, float, float, int]:
    """
    Nhan dien gioi tinh cua nguoi noi DAU TIEN du tin cay trong video.
    Tra ve: (gender: 'female'|'male'|'unknown', confidence: float, median_f0: float, seg_index: int)
    """
    if not srt_segments:
    ...
    for seg in srt_segments:
        ...
        idx = _get_index(seg, len(candidates) + 1)

        if start_s > max_search_time_sec and len(candidates) >= 1:
            break
        duration = end_s - start_s
        if duration >= MIN_SEGMENT_DURATION and re.search(r'\w', content):
            candidates.append((idx, start_s, end_s, content, duration))

    # Neu cac cau dau deu qua ngan, lay cau dai nhat trong nhung cau >= 0.25s
    if not candidates:
        for seg in srt_segments:
            start_s = _get_start_sec(seg)
            end_s = _get_end_sec(seg)
    ...
    if not candidates:
        logger.info("Khong tim thay doan phu de hop le o dau video.")
        return "unknown", 0.0, 0.0, -1

    best_candidate = ("unknown", 0.0, 0.0, -1)
    ...
    for idx, start_s, end_s, content, dur in candidates[:max_candidates]:
        audio_slice = None
        if primary_path:
            audio_slice = extract_segment_audio_slice(primary_path, start_s, end_s)

        # Neu vocal audio bi rong hoac loi, fallback sang original audio
        if audio_slice is None or np.max(np.abs(audio_slice[0])) < 1e-4:
            if secondary_path:
                audio_slice = extract_segment_audio_slice(secondary_path, start_s, end_s)

        if audio_slice is None:
            continue

        y, sr = audio_slice
        median_f0, voiced_count, conf, gender = estimate_f0_pitch(y, sr)

        logger.debug(
            "Doan #%d [%.2f-%.2f]: F0=%.1f Hz, voiced=%d, conf=%.2f, gender=%s",
            idx, start_s, end_s, median_f0, voiced_count, conf, gender
        )

        if conf >= CONFIDENCE_THRESHOLD:
            logger.info(
                "✅ Nhận diện thành công người nói đầu tiên: %s (F0=%.1f Hz, độ tin cậy=%.2f) ở câu #%d",
                "NỮ" if gender == "female" else "NAM", median_f0, conf, idx
            )
            return gender, conf, median_f0, idx

        if conf > best_candidate[1]:
            best_candidate = (gender, conf, median_f0, idx)

    if best_candidate[1] >= 0.60:
        logger.info(
            "Phat hien nguoi noi dau tien voi do tin cay chap nhan duoc: %s (F0=%.1f Hz, conf=%.2f)",
            best_candidate[0], best_candidate[2], best_candidate[1]
        )
        return best_candidate

    logger.info(
        "Khong nhan dien du tin cay nguoi noi dau tien (conf=%.2f, F0=%.1f Hz). Kich hoat fallback.",
        best_candidate[1], best_candidate[2]
    )
    return "unknown", best_candidate[1], best_candidate[2], best_candidate[3]
